/**
 * Regression for the usage half of #10: OpenCode reads `usage` as the
 * current context size, so a multi-call turn must report the latest API
 * call's totals (not the sum, not the SDK result's cumulative snapshot);
 * the sum rides along as `aggregate_usage`, and replayed assistant messages
 * from continuations count once.
 *
 * Run: bun test/usage-regression.ts
 */
...
console.log("ok — usage regression passed");
